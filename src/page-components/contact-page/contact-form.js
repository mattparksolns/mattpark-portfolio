...
const ContactForm = withStyles(theme => ({
  ...
  const handleSubmit = () => {
    if(!validator.isEmpty(name) && validator.isEmail(email) && !validator.isEmpty(message) && policyChecked) {
      ...
      axios.post("https://formspree.io/xnqbeekg", {
        name, email, message, policyChecked, userData,
      }).then((response) => {
  ...
  return (
    <form id={"#contactForm"} className={classes.form} onSubmit={handleSubmit}>
      <StyledTextField inputProps={{ 'aria-label': 'name' }} label={"Name"}
      ...
      <StyledTextField inputProps={{ 'aria-label': 'email' }} label={"Email"}
      ...
      <StyledTextField inputProps={{ 'aria-label': 'message' }} label={"Message"}
